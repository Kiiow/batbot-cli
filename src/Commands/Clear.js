...
class Clear extends Command{
  ...
  static async action(message) {
    ...
    UserDAL.getUserById(message.author.id)
      .then( async (user) => {
      ...
      .catch( (err) => {
        this.msg(message).sendError(`Erreur lors de la récupération de vos informations !`);
        this.log(0, `Erreur lors de la récupération de l'utilisateur`, err);
      })
